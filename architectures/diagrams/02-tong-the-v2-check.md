...
Đã đối chiếu với `../architecture_v3.md`, mục 2.2, 3.1, 4.1, 5 và 6. Workspace hiện là repository tài liệu, không có mã nguồn ứng dụng hay cấu hình triển khai để xác nhận hệ thống thực chạy. Kết luận chỉ áp dụng cho thiết kế được mô tả trong v3.
...
| AI/ML | Web/Text gọi Python khi bật, nhận probability/label/modelVersion | 2.2, 4.1 |
...
- RabbitMQ chỉ hiện nhóm queue cho scan/result/export. `q.notification`, `q.threat.ingest`, exchange/routing key, retry và DLQ chi tiết vẫn tra ở mục 5; ảnh không phải đặc tả đầy đủ topology.
- Ảnh theo tuyến AI ở mục 2.2. Khả năng Spring Boot gọi AI trực tiếp được nêu có điều kiện tại mục 2.3 không vẽ thêm để tránh rối.
...
- Xuất PNG trực tiếp từ SVG bằng Chrome ở tỉ lệ 2×: 3440 × 2940 px, không dùng thumbnail vuông của Quick Look rồi cắt ảnh.
