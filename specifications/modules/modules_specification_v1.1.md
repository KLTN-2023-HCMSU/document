...
| Phụ thuộc chính | PostgreSQL, Redis (nếu dùng session/revocation), M14 Audit & Observability |
...
- M09 chỉ liên quan khi cần admin policy/risk authorization.
...
| Input            | AI task có`scanId`, `taskId`, `kind`, payload/features/content                                  |
| Output           | `ai.analysis.completed` hoặc `ai.analysis.failed`                                                 |
| Business verdict | Không thuộc module này                                                                              |

## 2. Mục đích

Cung cấp model/LLM inference dùng chung cho URL/Text/Web analysis mà **không giữ Web/Text Worker phải chờ model**. M02/M03 publish AI task vào RabbitMQ; Python AI/ML Worker xử lý độc lập và trả prediction signal về `q.scan.result`. Final business verdict vẫn thuộc M09, còn việc chờ/ghép kết quả thuộc M12.
...
**M12 / Spring Boot** chịu trách nhiệm consume AI result, ghép bằng `scanId + taskId`, biến prediction thành AI `AnalysisSignal`, quản lý barrier và gọi M09. AI Worker không tự tạo `RiskResult`.
...
prediction = label + probability + modelVersion + latencyMs
...
M02/M03 chỉ cần `FakeMessageBus` để publish task; M12 có thể dùng AI result fixture mà không cần model thật; M11 có thể chạy contract test độc lập với RabbitMQ/Testcontainers.
...
Cung cấp lifecycle/orchestration dùng chung để URL/Text/Entity/QR không tự implement lại idempotency, queue dispatch, result consumption, nested scan, AI task tracking, completion barrier và finalization.
...
→ task đã biết: close task + convert prediction thành AnalysisSignal
