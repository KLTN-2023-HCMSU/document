...
> **Document version:** V1.3 — Identity & Notification Extensibility Alignment  
> **Architecture alignment:** V3.3 — theo [`../architectures/architecture_v3.3.md`](../architectures/architecture_v3.3.md).  
...
### Thay đổi chính của V1.3

- M01 chính thức hỗ trợ Guest + Local email/password + Google OIDC, đồng thời giữ external identity provider-agnostic qua `providerKey`/adapter registry.
- M01 bổ sung profile `fullName`, `displayName`, `dateOfBirth`, `avatarUrl`; `username` không phải field bắt buộc vì chưa có use case public handle/unique handle.
- M01 bổ sung email verification, forgot/reset password và password-change OTP challenge; challenge ownership ở M01, delivery đi qua M10.
- M01 chốt auth-session model: **JWT access token ngắn hạn + opaque rotating refresh token + server-side `auth_sessions`**; refresh reuse revoke cả token family/session và phát security notification. Version tài liệu vẫn giữ V1.3.
- M10 chuyển thành notification capability channel/provider-agnostic; `IN_APP`, `EMAIL`, `PUSH` là channel hiện tại, còn `SMS`, `TELEGRAM`, `WHATSAPP`, `ZALO`, ... có thể thêm mà không đổi business-module contract.
- V1.3 không thay đổi scan/AI/Risk Fusion contract của M02–M09/M11–M14.
...
- tài khoản Free được dùng để sở hữu **persistent scan history**, profile/session, quota theo account và các tính năng follow-up cần danh tính; authenticated account có thể dùng Local email/password hoặc Google OIDC nhưng downstream vẫn chỉ nhận cùng `AccessContext`;
...
| Backend owner | Auth, User, Identity Provider Adapter & Access Context domain |
| Data owner | `users`, `auth_identities`, `local_credentials`, `auth_verification_challenges`, `roles`, `user_roles`, session/refresh metadata |
| MVP auth methods | Guest, Local email/password, Google OAuth 2.0 / OpenID Connect |
| MVP session/token model | JWT access token ngắn hạn + opaque refresh token rotate + server-side `auth_sessions`; PostgreSQL source of truth, Redis cache/revocation lookup |
| Future auth extension | thêm external identity provider bằng `providerKey` + adapter/config, không đổi `AccessContext` |
| Future commercial extension | plan/entitlement, user API identity, subscription/credit/usage — **không thuộc MVP** |
| Phụ thuộc chính | PostgreSQL, Redis (session/revocation/quota/rate-limit), M10 Notification Delivery, M14 Audit & Observability |

## 2. Mục đích

Cung cấp identity, credential verification, external identity integration, **JWT access token + rotating refresh token + server-side auth session**, RBAC, profile và **AccessContext** thống nhất cho User Web, Mobile và Admin. M01 không được biến authentication thành điều kiện bắt buộc để scan: Guest vẫn có thể dùng các scan endpoint được policy cho phép.

M01 phân tách **account** khỏi **authentication method**. Một `users` record có thể có Local credential, Google identity hoặc cả hai; scanner/AI/business module khác không được biết user đăng nhập bằng cách nào. Sau khi authentication thành công, M01 chỉ phát `AccessContext`/`AuthenticatedPrincipal` chuẩn hóa.

Email verification, password reset và password change challenge thuộc M01. M01 tạo/verify challenge, quản lý hash/expiry/attempt/single-use; **M10 chỉ chịu trách nhiệm delivery OTP/security notification**, không tự xác thực OTP.
...
**Local account — đăng ký + verify email**

```text
Nhập email + password + profile tối thiểu
→ M01 tạo account ở trạng thái/chế độ chưa verify email
→ M01 tạo EMAIL_VERIFICATION challenge
→ yêu cầu M10 gửi OTP qua EMAIL
→ user nhập OTP
→ M01 verify challenge + set emailVerifiedAt
→ tạo `auth_session`
→ cấp Anti-Scam JWT access token + opaque refresh token
```

**Local account — đăng nhập / quên mật khẩu / đổi mật khẩu**

```text
Login bằng email + password
→ persistent History / profile / quota theo account

Forgot Password
→ nhập email
→ luôn nhận response trung tính, không lộ account tồn tại hay không
→ nếu account hợp lệ: M01 tạo PASSWORD_RESET challenge
→ M10 gửi OTP
→ M01 verify OTP + đổi password
→ revoke/rotate session theo policy

Change Password
→ authenticated user xác nhận current password
→ M01 tạo PASSWORD_CHANGE challenge
→ M10 gửi OTP
→ verify OTP + đổi password
→ gửi security notification
```

**Google OIDC — MVP external provider đầu tiên**

```text
Chọn “Continue with Google”
→ client nhận authorization result / provider assertion theo flow đã chọn
→ Spring Boot M01 verify Google OIDC response
→ lookup (providerKey='google', providerSubject=<Google sub>)
→ create/link account theo account-linking policy
→ normalize thành cùng AuthResult + AccessContext
→ tạo `auth_session` + cấp Anti-Scam JWT access token / opaque refresh token
→ downstream không biết user đăng nhập bằng Google hay Local; Google token không dùng để gọi Anti-Scam API
```
...
- Local register/login/logout bằng email/password;
- Google OAuth 2.0 / OpenID Connect login qua external identity adapter;
- external identity registry theo `providerKey`, không hard-code provider vào downstream business contract;
- email verification bằng OTP/challenge;
- forgot/reset password bằng OTP/challenge;
- authenticated password change với current-password proof + OTP challenge theo policy;
- JWT access token ngắn hạn mang `sub`/`sid`/`roles` + issuer/audience/expiry;
- opaque refresh token chỉ lưu hash, rotate mỗi lần refresh, token-family reuse detection;
- server-side `auth_sessions` theo device/login, hỗ trợ revoke một phiên hoặc tất cả phiên;
- refresh/session lifecycle, rotation/revoke;
- xem/chỉnh profile: email, full name, display name, date of birth, avatar;
...
- account linking có kiểm soát giữa Local và external identity.

**Không yêu cầu trong MVP:**
- `username`/public handle unique vì hiện không có feature nào cần `@username` hoặc profile URL public;
- SMS/Telegram OTP: M01 chỉ phụ thuộc NotificationPort, nên có thể bật sau khi M10 có channel/provider tương ứng mà không đổi challenge logic.

**Future Scope — chỉ thiết kế extension point, chưa implement ở MVP:**
- external identity provider khác Google;
- plan/subscription;
- paid entitlement;
...
- màn hình nhập OTP + resend countdown cho email verification/password challenge;
- Forgot Password / Reset Password;
- Change Password;
- Profile: `fullName`, `displayName`, `dateOfBirth`, `avatarUrl`, email/verification status;
- Logout + quản lý session cơ bản nếu scope có;
- Web: access token ưu tiên giữ trong memory; refresh credential qua `HttpOnly + Secure + SameSite` cookie, không dùng `localStorage` cho refresh token;
...
- hiển thị lỗi 401/403/expired challenge hợp lý.
...
POST  /v1/auth/login
POST  /v1/auth/external/{providerKey}/exchange     # MVP providerKey=google
...
POST  /v1/auth/password/change/request-otp
...
Logical auth result cho Local/Google sau khi authentication thành công:
...
- refreshCredential: opaque rotating token
  - Web có thể deliver bằng HttpOnly cookie thay vì JSON body
  - Mobile nhận logical credential và lưu secure storage
- sessionId (`sid`)
- user / roles / AccessContext
```

JWT access token tối thiểu:
...
`POST /v1/auth/refresh` phải rotate refresh token. Token cũ đã consumed/replaced mà xuất hiện lại được coi là reuse attack: revoke token family + session và trả 401; M01 có thể yêu cầu M10 phát security notification.

`/external/{providerKey}/exchange` là contract logic: payload cụ thể có thể là authorization code, ID token/provider assertion hoặc server-side callback result tùy Web/Mobile flow. Adapter của provider chịu trách nhiệm verify issuer/audience/signature/nonce/state theo OIDC/OAuth flow đã chọn. Business module khác không thấy payload provider-specific.
...
AuthMethod
- kind = LOCAL | EXTERNAL
- providerKey = null | google | <future extensible key>
...
M01 gọi một `NotificationPort` nội bộ hoặc publish `notification.requested` thông qua M10 để gửi OTP/security message. Notification contract chỉ vận chuyển message; M01 vẫn là source of truth của challenge.

`planKey`, `quotaProfile`, `inferenceProfile`, `providerKey` là extensible config keys; không hard-code billing/AI/auth provider vào downstream contract.

## 7. Backend / worker responsibilities

**Backend**
- canonicalize email (`trim` + lowercase policy) trước lookup/persist;
- validate Local credential, hash/verify password;
- external identity adapter registry; MVP adapter Google OIDC;
- verify provider issuer/audience/signature/state/nonce theo flow;
- dùng `(providerKey, providerSubject)` làm external identity key; với Google, `providerSubject` là `sub`, không dùng email làm external identity chính;
- create/link user theo account-linking policy;
- tạo `auth_sessions` theo login/device;
- issue JWT access token ngắn hạn; verify signature/issuer/audience/expiry và resolve `sid`;
- issue opaque refresh token, chỉ persist hash; rotate mỗi lần refresh; detect token reuse và revoke family/session;
- cache session/revocation state ở Redis nhưng PostgreSQL là source of truth;
- logout/revoke một session hoặc tất cả session theo security action;
...
- load/update profile;
- tạo challenge, hash OTP, expiry, attempt limit, single-use consume;
- gọi M10 NotificationPort cho email verification/password/security delivery;
- enforce RBAC;
- propagate resolved access/entitlement context cho M12 và account-aware modules.

**Worker**
- không có scanner/AI worker riêng cho M01;
- notification delivery worker thuộc M10, không thuộc M01;
- scanner/AI worker không nhận raw token, password, OTP, date of birth, auth provider assertion, plan/payment data hoặc credit balance.

## 8. Input

```text
Local credential: email + password
External identity assertion / authorization result
JWT access token; opaque refresh token; `sid`/server-side auth session
guest session/access token nếu có
verification/reset/change-password OTP
...
```text
AuthResult (JWT access token + rotated refresh credential + sessionId)
UserProfile
AccessContext
...
Roles / Permissions
401 / 403 / standardized auth errors
```
...
- `users` giữ account/profile, không giữ password hash;
- `local_credentials` chỉ tồn tại khi account có Local password login;
- `auth_identities` giữ external provider identity; Google là row `provider_key='google'` + `provider_subject=<sub>`;
- `auth_verification_challenges` là source of truth của OTP purpose/hash/expiry/attempt/single-use;
- `auth_sessions` là source of truth của login session theo device, expiry/revocation và security logout;
- `refresh_tokens` chỉ giữ hash + rotation lineage/token family; không có bảng `access_tokens` vì access token là JWT ngắn hạn;
- Guest identity là ephemeral; không tạo `users` record chỉ để cho phép một lần scan.
...
- `users.email` được canonicalize trước persistence và unique case-insensitive;
- `displayName` là tên hiển thị, **không phải username** và không yêu cầu unique;
- `username` không tồn tại trong MVP nếu chưa có public-handle use case;
- `dateOfBirth` là profile data nhạy cảm: không đưa vào scan event, worker payload, AI prompt/result hoặc log;
- password hash chỉ nằm trong `local_credentials`, không nằm trong `users`;
- Google-only account không bắt buộc có Local credential/password;
- external identity unique theo `(providerKey, providerSubject)`; Google dùng `sub` làm subject;
- không silent-link account chỉ vì email trùng. Nếu external login trùng email account hiện hữu, policy phải yêu cầu proof/linking an toàn hoặc trả `ACCOUNT_LINK_REQUIRED`;
- nếu provider khẳng định email đã verified và policy tin provider đó, M01 có thể set `emailVerifiedAt`; không gửi OTP lần nữa;
- OTP/challenge chỉ lưu hash, có expiry + max attempts + single-use; resend tạo challenge/code mới hoặc invalidate code cũ theo policy;
- forgot-password response không được tiết lộ email/account có tồn tại hay không;
- sau reset/change password phải revoke/rotate session theo security policy;
...
- access token của MVP là JWT ngắn hạn; mặc định 15 phút và cấu hình được;
- JWT phải có `sid`; authenticated API resolve session state theo `sid` qua Redis cache, cache miss về PostgreSQL; session `REVOKED/EXPIRED` → 401 dù JWT chưa hết `exp`;
- refresh token là opaque random credential, mặc định 30 ngày và **rotate mỗi lần refresh**; raw token không persist;
- mọi refresh token cùng một lineage có `token_family_id`; token cũ đã `used/replaced` mà được trình lại → reuse detected → revoke cả family + session + security notification;
- logout revoke session hiện tại và refresh family; logout-all revoke toàn bộ session của user;
- password reset revoke toàn bộ session; authenticated password change mặc định revoke session khác và rotate current refresh credential;
- role/permission change cần hiệu lực ngay phải revoke session bị ảnh hưởng để access token cũ không tiếp tục mang role stale;
- Admin không mặc định có toàn quyền nếu hệ thống dùng granular permissions;
- các module khác không tự đọc credential/provider assertion hay tự verify token;
...
- sai Local credential → fail fast bằng error chuẩn, không lộ field nào sai;
- external provider unavailable/invalid assertion → fail auth rõ ràng, không fallback tự động sang account khác;
- email collision khi external sign-in nhưng chưa link an toàn → `ACCOUNT_LINK_REQUIRED`, không silent merge;
- challenge expired/consumed/max-attempts → reject; user có thể request challenge mới theo rate-limit policy;
- M10/provider email lỗi → account/challenge state vẫn giữ đúng; cho phép resend bounded, không mark verified vì delivery fail;
- access token hết hạn → dùng refresh flow; refresh không hợp lệ/hết hạn → login lại;
- refresh token đã used/replaced xuất hiện lại → coi là reuse attack, revoke token family + session, trả 401 và phát security notification;
- session bị revoke/expired → access JWT bị từ chối theo `sid` dù chữ ký/`exp` còn hợp lệ;
- Redis session/quota/rate-limit cache unavailable → fallback theo source of truth/policy nếu architecture cho phép; không tự nâng quyền;
- entitlement resolver future unavailable → không tự nâng premium;
- không retry credential mutation một cách mù quáng.

## 13. Security / privacy

- password dùng adaptive hash phù hợp; raw password không persist/log;
- OTP chỉ persist dưới dạng hash; raw OTP chỉ tồn tại transient để delivery, không copy vào audit/`notification_jobs.payload_json`/DLQ/log;
- external identity token/code/assertion không persist trong business DB và không log;
- OIDC phải validate issuer, audience, signature, expiry và anti-CSRF/replay controls tương ứng flow (`state`/`nonce` khi áp dụng);
- refresh token/session metadata phải có revoke/expiry/rotation;
- access JWT dùng signing key từ secret/key management, có `kid` để hỗ trợ rotation; signing key không lưu trong business DB;
- raw access token không persist; raw refresh token không persist, DB chỉ lưu hash;
- refresh rotation phải atomic để tránh concurrent double-refresh; reuse detection revoke family/session;
- Web không lưu refresh token trong `localStorage`; ưu tiên HttpOnly+Secure+SameSite cookie. Mobile dùng OS secure storage;
- nếu JWT signing key bị compromise: rotate key, ngừng chấp nhận key cũ theo incident policy và revoke toàn bộ affected sessions;
- challenge rate limit theo IP + account/email fingerprint; giới hạn attempt và resend;
- account enumeration protection cho login recovery/forgot-password;
- guest session ID random/opaque, không dùng fingerprint ổn định làm identity chính;
- `fullName`, `dateOfBirth`, email và endpoint liên lạc phải authorize/mask phù hợp;
- profile data không truyền sang scanner/AI nếu không phải input scan do user chủ động cung cấp;
...
- **M10 Notification & User Follow-up**: M01 gọi `NotificationPort` để gửi verification/password/security message; M10 không sở hữu challenge truth;
- M12 Shared Scan Platform nhận `AccessContext`/`EntitlementContext`;
- M13 Platform Infrastructure cho session/quota/rate-limit/secret config;
- M14 Audit & Observability cho security-sensitive actions nhưng audit phải redact credential/OTP/provider assertion;
- Google/external identity provider là external dependency sau adapter boundary;
...
mock-user-local.json
mock-user-google.json
mock-admin.json
mock-guest.json
mock-expired-session.json
mock-expired-challenge.json
```

Frontend có thể hoàn thiện guest scan, Local/Google login, OTP/profile/history gate bằng mock adapter trước khi provider/backend hoàn tất. M12 chỉ consume fake `guest`/`free` access/entitlement context; không phụ thuộc auth method hay billing implementation.

## 16. Definition of Done

- Guest có thể scan các public scan type mà không cần register/login;
- Local register/login/logout + email verification hoạt động;
- Forgot/Reset Password và Change Password OTP challenge hoạt động với expiry/attempt/single-use;
- Google OIDC sign-in hoạt động qua adapter và normalize về cùng `AccessContext`;
- account-linking collision không silent merge;
- Web + Mobile dùng cùng auth/access/profile contract;
- `UserProfile` hỗ trợ `fullName`, `displayName`, `dateOfBirth`, `avatarUrl`; không bắt buộc `username`;
...
- integration test guest/local/google + login/refresh/logout + challenge flow;
- refresh rotation + token-family reuse detection có integration test;
- revoke session khiến access JWT cùng `sid` bị reject; logout-all/password-reset revoke đúng phạm vi session;
- Web/Mobile token-storage policy được kiểm thử/scan để không lưu raw refresh credential ở nơi không an toàn;
- `AccessContext` + `EntitlementContext` có mock/contract test;
- M01↔M10 contract test chứng minh delivery failure không làm challenge được verify sai;
- MVP không có dependency bắt buộc vào billing/credit/subscription/user API key;
- không có credential/token/OTP/provider assertion trong log/audit/message DLQ lâu dài;
...
| Actor | User, Admin; producer nội bộ gồm M01/M06/M07/M12/M14 |
| UI | Web/Mobile notification center/badge/settings nếu scope có |
| Backend | Notification policy, channel resolver, job dispatcher |
| Worker | Notification delivery worker + provider adapter registry |
| Data owner | `notification_preferences`, `notification_endpoints`, `notification_jobs`, `notification_deliveries` |
| Current channels | `IN_APP`, `EMAIL`, optional `PUSH` |
| Future channels | `SMS`, `TELEGRAM`, `WHATSAPP`, `ZALO`, `DISCORD`, ... bằng adapter/config, không đổi producer contract |

## 2. Mục đích

Cung cấp một **notification delivery platform channel/provider-agnostic** để các module phát business/security event mà không tự tích hợp SMTP, FCM, SMS, Telegram hoặc vendor cụ thể.

M10 tách ba khái niệm:

```text
Notification Type = business reason cần thông báo
Channel           = IN_APP | EMAIL | PUSH | SMS | TELEGRAM | ...
Provider          = adapter/vendor thực hiện delivery cho channel đó
```

Thêm một channel/provider mới không được buộc M01/M06/M07/M12 sửa business logic; producer chỉ gửi `NotificationRequest`, M10 resolve policy/preference/endpoint/provider và chịu trách nhiệm delivery.
...
→ M10 resolve channel theo policy + user preference
→ nhận in-app/email/push hiện tại
→ tương lai có thể nhận SMS/Telegram/... nếu user đã cấu hình endpoint
→ tap/deep-link tới đúng resource và backend authorize lại
```

**Transactional auth/security message từ M01**

```text
Register / Forgot Password / Change Password
→ M01 tạo verification challenge
→ M01 gửi NotificationRequest cho M10
→ M10 deliver OTP/security message qua EMAIL hiện tại
→ tương lai policy có thể chọn SMS/Telegram nếu được phép
→ M01 mới là thành phần verify OTP/challenge
```

## 4. Phạm vi nghiệp vụ

**Notification types hiện tại / MVP-oriented:**
- `AUTH_EMAIL_VERIFICATION`;
- `AUTH_PASSWORD_RESET`;
- `AUTH_PASSWORD_CHANGE_OTP`;
- `AUTH_PASSWORD_CHANGED` / security alert;
- `SCAN_COMPLETED`;
- `EXPORT_COMPLETED`;
- `REPORT_REVIEWED`;
- `SYSTEM_ALERT`.
...
- `EMAIL`;
- optional `PUSH`;
- future `SMS`, `TELEGRAM`, `WHATSAPP`, `ZALO`, `DISCORD`, ...;
- read/unread state cho in-app nếu scope có;
- deep-link target;
- user preferences theo channel/event;
- endpoint registration/verification/disable;
- template resolution bằng `templateKey`;
- bounded retry, provider fallback nếu policy cho phép;
- delivery trace/status.

M10 **không** sở hữu business truth của OTP, scan, report hay export; nó chỉ deliver dựa trên request/event hợp lệ.
...
- read/unread;
- deep-link;
- notification settings theo channel/event nếu scope có;
- quản lý endpoint/channel user-configurable nếu feature được bật;
- permission UX cho push trên mobile/web;
- trạng thái verify/disable endpoint phù hợp;
- không cho user bật channel chưa có endpoint hợp lệ nếu channel đó cần endpoint.

Auth OTP screen thuộc M01 UI; M10 chỉ cung cấp delivery status/resend support phía backend.

## 6. API / event contracts

Input business/security events có thể gồm:

```text
AuthEmailVerificationRequested
AuthPasswordResetRequested
AuthPasswordChangeOtpRequested
AuthPasswordChanged
ScanCompleted
ExportCompleted
ReportReviewed
...
GET   /v1/notification-endpoints
POST  /v1/notification-endpoints
PATCH /v1/notification-endpoints/{id}
...
Canonical internal `NotificationRequest`:
...
- notificationType
- recipientUserId
- channelHints[]          # optional; policy có quyền override/fallback
- templateKey
- templateData            # NON-SENSITIVE merge data only
- resourceType / resourceId
- deepLinkPath
- priority
- idempotencyKey
- expiresAt               # đặc biệt cho OTP/transactional message
- sensitivePayload        # transient/encrypted transport only, NEVER persisted/logged
```

`channelKey`, `providerKey`, `templateKey` là extensible string/config key, không dùng closed database enum bắt buộc migration mỗi khi thêm channel/provider.

## 7. Backend / worker responsibilities

```text
Domain/Security Event or NotificationRequest
→ Notification Policy
→ Preference Resolver
→ Endpoint Resolver
→ create per-channel NotificationJob
→ Channel Resolver
...
**Backend/dispatcher**
- validate notification type/recipient/resource;
- resolve channel theo mandatory policy + user preference;
- resolve endpoint đã đăng ký/verified theo policy;
- tạo `notification_jobs` idempotent;
- không persist raw OTP/secret trong `payload_json`;
- publish `notification.requested`.

**Delivery worker/provider adapter**
- chọn provider theo `channelKey` + config/health/fallback policy;
- render template theo channel;
- lấy/decrypt transient sensitive payload chỉ tại delivery boundary nếu request cần OTP;
- gửi, bounded retry, provider fallback theo policy;
- ghi `notification_deliveries`;
- không tự thay đổi business state của M01/M06/M07/M12.

Provider examples chỉ là implementation, không phải contract: SMTP/SES/Resend/SendGrid cho email, FCM/APNs/WebPush cho push, SMS gateway, Telegram Bot API, ...

## 8. Input

```text
NotificationRequest / Domain Event
recipient userId/admin target
notificationType
templateKey + non-sensitive template data
related resource id / deep-link
channel hints / mandatory channel policy
notification preferences
notification endpoints
provider config/secret reference
optional transient encrypted sensitive payload (OTP)
```
...
Producer nhận delivery status khi cần nhưng notification delivery failure không được giả thành business success/failure của producer domain.
...
- `notification_preferences`: user muốn nhận loại thông báo nào qua channel nào;
- `notification_endpoints`: địa chỉ đích generic theo channel (`email`, phone, push token, Telegram chat id, ...), lưu encrypted/protected + fingerprint;
- `notification_jobs`: một job logic theo recipient + channel + notification type; giữ presentation/deep-link/template metadata **không nhạy cảm**;
- `notification_deliveries`: từng attempt qua provider/endpoint, status + external message id + sanitized error.

Provider credential/API token nằm secret/config management, không nằm các bảng trên.

## 11. Business rules / invariants

- notification failure không rollback scan/report/export/account mutation đã commit;
- riêng verification/password flow: delivery failure không được mark challenge verified/consumed;
- không gửi duplicate cho cùng `idempotencyKey`;
- producer không hard-code SMTP/Twilio/Telegram/vendor SDK;
- business module không branch theo `providerKey`;
- channel và provider là hai khái niệm khác nhau;
- mỗi channel mới phải implement adapter contract + endpoint/template capability, không sửa producer event schema;
- in-app notification có thể không cần external endpoint;
- email/SMS/push/Telegram/... cần endpoint hợp lệ theo channel policy;
- user preference không được tắt security/mandatory notification nếu policy đánh dấu mandatory;
- deep-link phải authorize lại khi user mở;
- raw OTP/credential không nằm trong canonical notification tables;
- expired transactional notification không được gửi sau `expiresAt`;
- provider fallback không tạo duplicate user-visible message nếu provider đầu đã xác nhận thành công.

## 12. Failure / degraded behavior

- provider down → bounded retry, sau đó fallback provider cùng channel nếu policy cho phép;
- endpoint invalid/unverified → skip channel hoặc fallback channel theo policy, không đoán endpoint;
- retry exhausted → failed status/DLQ/sanitized alert;
- push denied → in-app/email vẫn có thể hoạt động theo policy;
- SMS/Telegram provider chưa cấu hình → channel unavailable, không ảnh hưởng channel khác;
- duplicate event → ignore/idempotent;
- `expiresAt` qua hạn → discard transactional message, không retry tiếp;
- M10 unavailable trong auth OTP flow → M01 giữ challenge đúng trạng thái và cho resend bounded; không tự verify.

## 13. Security / privacy

- không đưa sensitive scan detail vào push/SMS preview nếu không cần;
- deep-link vẫn yêu cầu auth/authorization;
- provider credential không log và nằm secret manager/environment;
- endpoint address (email/phone/chat id/push token) phải encrypt/protect at rest khi phù hợp; log chỉ dùng masked/fingerprint;
- raw OTP/password reset code không persist trong `notification_jobs`, `notification_deliveries`, audit hoặc long-lived DLQ;
- sensitive delivery payload nếu async phải application-encrypted/transient + message TTL và bị drop khi expired;
- template rendering phải chống injection phù hợp channel;
- notification preferences/endpoints authorize theo user;
- webhook/delivery callback từ provider nếu có phải verify signature/source và idempotent.

## 14. Dependencies

- M01 Identity phát verification/password/security `NotificationRequest` nhưng giữ challenge source of truth;
- M06 export/result;
- M07 reports;
- M12 messaging/outbox;
- M13 infrastructure cho RabbitMQ, secret/config, optional transient encrypted payload store;
- M14 Audit & Observability cho delivery/failure trace;
- external notification providers nằm sau adapter boundary.

M10 chỉ cần `userId`/endpoint data của chính nó; không gọi ngược M01 để verify credential/OTP.
...
mock-auth-otp-notification.json
mock-scan-completed-event.json
mock-report-reviewed-event.json
mock-provider-failure.json
```

UI notification center dùng seeded notifications. M01 dùng `MockNotificationPort` để test challenge flow mà không cần SMTP/Google/provider thật. Provider adapter có contract test chung để thêm channel/provider mới không phá M10 core.

## 16. Definition of Done

- in-app notification flow hoạt động;
- transactional auth email cho verify/reset/change-password hoạt động mà M10 không verify OTP;
- email provider có adapter contract + delivery trace;
- channel/provider được tách bằng `channelKey`/`providerKey`;
- thêm mock SMS/Telegram adapter không yêu cầu sửa producer contract/schema business;
- endpoint registration/disable/authorization hoạt động nếu channel cần endpoint;
- deep-link đúng và authorize lại;
- duplicate protection/idempotency;
- expired OTP notification không được gửi;
- raw OTP/secret không tồn tại trong canonical notification DB/log/audit/DLQ lâu dài;
- provider failure không ảnh hưởng business transaction;
- delivery status observable;
- provider fallback test không tạo duplicate message sau success.
